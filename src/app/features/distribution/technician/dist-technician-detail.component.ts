...
@Component({
    selector: 'app-dist-technician-detail',
    standalone: true,
    imports: [
        BlockUIModule,
        ButtonModule,
    ...
    template: `
        ...
        <div class="card">
            @defer (when true) {
                ...
                @if (false) {
               <!--  @if (responseRequestDetail()?.solicitudes?.estado !== 'Asignada') { -->
                    <p-messages
                        styleClass="mt-4"
                                    ...
                                    <div class="field col-12  md:col-6 xl:col-4">
                                        ...
                                        <div class="custom-file-button">
                                            ...
                                            <input
                                                type="file"
                                                id="photoNode"
                                                (change)="onPhotoNodeSelected($event)"
                                                name="file"
                                                accept="image/*" />
                                        </div>
                                        @if (photoNodeErr) {
                                    ...
                                    <div class="field col-12  md:col-6 xl:col-4">
                                        ...
                                        <div class="custom-file-button">
                                            ...
                                            <input
                                                type="file"
                                                id="photoTransformer"
                                                (change)="onPhotoTransformerSelected($event)"
                                                name="file"
                                                accept="image/*" />
                                        </div>
...
export class DistTechnicianDetailComponent implements OnInit, OnDestroy {
     ...
     *
     */
    private configureFormForTypeRequest () {
        const fields = [
        ...
        if (this.responseRequestDetail()?.solicitudes.tipo_display !== 'Disponibilidad') {
            console.log('entro');
            fields.forEach(field => this.visitForm.get(field)?.disable());
        }
        console.log(this.responseRequestDetail());
    }

    /**
     * Handles the file selection event for the photo node.
     * @param {Event} event - The file input change event.
     */
    public onPhotoNodeSelected (event: Event): void {
        ...
        if (input.files && input.files.length > 0) {
            if (
                ...
                this.showAlert('error', 'Error', 'El formato del archivo no es compatible');
                return;
            }
            if (input.files[0].size > 5242880) {
                this.showAlert('error', 'Error', 'El archivo supera el tamaño máximo permitido de 5 MB');
                return;
            }
            this.photoNodeToUpload = input.files[0];
            this.photoNodeName = this.utilsService.truncateStringWithEllipsis(this.photoNodeToUpload.name);
            this.photoNodeErr = false;

            const reader = new FileReader();
            reader.onload = (e: ProgressEvent<FileReader>) => {
                this.previewPhotoNode.set(e.target?.result as string);
            };
            reader.readAsDataURL(this.photoNodeToUpload);
        } else {
            this.photoNodeErr = true;
            this.photoNodeName = null;
            this.photoNodeToUpload = null;
            this.previewPhotoNode.set(null);
        }
    }
    /**
     * Handles the file selection event for the photo transformer.
     * @param {Event} event - The file input change event.
     */
    public onPhotoTransformerSelected (event: Event): void {
        const input = event.target as HTMLInputElement;
        if (input.files && input.files.length > 0) {
            if (
                !input.files[0].type.startsWith('image/') ||
            ...
            this.photoTransformerToUpload = input.files[0];
            this.photoTransformerName = this.utilsService.truncateStringWithEllipsis(
                this.photoTransformerToUpload.name
            );
            this.photoTransformerErr = false;

            const reader = new FileReader();
            reader.onload = (e: ProgressEvent<FileReader>) => {
                this.previewPhotoTransformer.set(e.target?.result as string);
            };
            reader.readAsDataURL(this.photoTransformerToUpload);
        } else {
            this.photoTransformerErr = true;
            this.photoTransformerName = null;
            this.photoTransformerToUpload = null;
            this.previewPhotoTransformer.set(null);
        }
    }
    /**
     ...
     * @param {string} detail
     */
    public showAlert (severity: string, summary: string, detail: string) {
     ...
     */
    public endVisit () {
        this.submVisitForm = true;

        const fecha_visita_inicio_format = formatDate(
            this.visitForm.get('fecha_visita_inicio')?.value,
            'yyyy-MM-ddTHH:mm',
            'en-US',
            '-0500'
        );
        this.visitForm.get('fecha_visita_inicio')?.setValue(fecha_visita_inicio_format);
        const fecha_visita_fin_format = formatDate(
            this.visitForm.get('fecha_visita_fin')?.value,
            'yyyy-MM-ddTHH:mm',
            'en-US',
            '-0500'
        );
        this.visitForm.get('fecha_visita_fin')?.setValue(fecha_visita_fin_format);
        const corriente_fecha_lectura_format = formatDate(
            this.visitForm.get('corriente_fecha_lectura')?.value,
            'yyyy-MM-ddTHH:mm',
            'en-US',
            '-0500'
        );
        this.visitForm.get('corriente_fecha_lectura')?.setValue(corriente_fecha_lectura_format);

        if (!this.photoNodeToUpload) {
            this.photoNodeErr = true;
        }
        if (!this.photoTransformerToUpload) {
            this.photoTransformerErr = true;
        }
        if (!this.fotoFachadaToUpload) {
            this.fotoFachadaErr = true;
        }
        if (this.visitForm.invalid || this.photoNodeErr || this.photoTransformerErr || this.fotoFachadaErr) {
            return;
        }

        this.LoadingRequestVisit = true;
        console.log(this.visitForm.value); //TODO: Eliminar
        const processedData = this.utilsService.convertFormValuesToStrings<DataVisitDist>(this.visitForm.getRawValue());
        this.distVisitService.formVisitDist.set({
            ...this.distVisitService.formVisitDist(),
            datos: processedData,
        });
        console.log(processedData); //TODO: Eliminar

        this.distVisitService.formVisitDist().nodo_foto = this.photoNodeToUpload;
        this.distVisitService.formVisitDist().transformador_foto = this.photoTransformerToUpload;
        this.distVisitService.formVisitDist().fachada_foto = this.fotoFachadaToUpload;

        console.log(this.distVisitService.formVisitDist()); //TODO: Eliminar

        this.distVisitService.sendDataVisitDist().subscribe({
            next: () => {
                this.LoadingRequestVisit = false;
                this.messageService.add({
                    severity: 'success',
                    summary: 'Datos enviados',
                    detail: 'Se han enviado los datos correctamente',
                });
    ...
    public ngOnDestroy (): void {
        ...
        this.distVisitService.cancelRequests();
    }
}
